...
const AdminSidebar = () => {
    ...
    // tránh lỗi re-render infinite loop
    const toggleCollapse = () => {
        setCollapsed(!collapsed);
        useContext(UserContext).setCollapsed(!collapsed);
      };
    ...
    return (
        ...
        <div className="flex-grow overflow-y-auto py-2">
            ...
            {menuItems.map((item) => (
                ...
                <button
                    ...
                    activeMenu === item.path ? 'bg-gray-700 text-blue-400' : 'text-gray-300 hover:bg-gray-700'
                ...
                >
                    ...
                    <ChevronDown
                        size={16}
                        className={`transition-transform duration-300 ${expandedMenus.includes(item.path) ? 'rotate-180' : ''}`}
                    />
                    )}
                </button>
                
                {/* Submenu */}
                {!collapsed && item.submenu && expandedMenus.includes(item.path) && (
        ...
        <div className="p-4 border-t border-gray-700">
            <Link className={`flex items-center ${collapsed ? 'justify-center w-full' : ''} text-red-400 hover:text-red-300`}
                to="/logout"
            >
                <LogOut size={20} />
