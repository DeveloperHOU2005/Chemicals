...
import statusClasses from '../../../types/productStatus';
const Detail = ({ isOpen, onClose, selectedCategory }) => {
  ...
  useEffect(() => {
    const getProductsInCategory = async () => {
      setLoading(true);
      try {
        const response = await productApi.getProductsByCategory(selectedCategory.id);
                  ...
                  <div className="w-full">
                    ...
                    <div className="flex items-center space-x-2 mb-4">
                      <span className={`px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full ${
                        categoryDetails.trangthai === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                      }`}>
                        {categoryDetails.trangthai === 'active' ? (
                      ...
                      <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {productsInCategory.map(product => (
                            <tr key={product.id} className="hover:bg-gray-50" onClick={handleProductClick(e, product.id)}>
